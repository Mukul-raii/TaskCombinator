...
const createTask= asyncHandler(async (req, res,next) => {
    const{taskName,taskDescription,taskStatus,taskDueDate,taskPriority,teamId,assignTo}= req.body
    ...
    if(!taskName || !taskDescription || !taskStatus || !taskDueDate || !taskPriority ||!teamId){
        return res.send(new apiError(400, "Please provide task name, task description, task status, task due date, task priority"));
    }
    ...
    try {
        ...
        return res.send(new apiResponse(201, "Task created successfully", taskDetails));
...
const updateTask=asyncHandler(async (req, res) => {
    ...
    const{taskName,taskDescription,taskStatus,taskId,taskDueDate,taskPriority,teamId,assignTo}= req.body
    ...
    const team=await Team.findOne({teamId:task.teamId})
        ...
        try {
            const taskDetails= await Task.findByIdAndUpdate({_id:taskId},{
                taskName,
                taskDescription,
                taskStatus,
                taskDueDate,
...
const getAllTasks= asyncHandler(async (req, res) => {
    ...
    return res.send(new apiResponse(200, "Tasks retrieved successfully", tasks));
})
