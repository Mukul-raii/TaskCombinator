        ...
        <div className='bg-white border bottom-1 text-white flex justify-center mx-2 rounded-full px-6 h-16 xs:max-md:px-1 xs:max-md:h-14 shadow-white  fixed  top-0 w-full z-10'>
            <div className='flex justify-between items-center w-full mx-auto'>
                <div className='flex items-center flex-row gap-2'>
                    ...
                    <h1 className='text-xl font-bold text-blue-950 cursor-pointer' onClick={() => navigate('/')}>
                        TaskCombinator
                    </h1>
                </div>

                {/* Right-side buttons */}
                <div className='flex space-x-4'>{state.isAuthenticated ? <UserBtn state={state} /> : <LoginBtn />}</div>
            </div>
        </div>
    )
}
